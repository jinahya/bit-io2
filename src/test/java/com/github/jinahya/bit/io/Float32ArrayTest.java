...
class Float32ArrayTest {
    ...
    void testSigned(@ConvertWith(ByteOutput2BitOutputConverter.class) final BitOutput output,
        ...
        try (BitOutput o = output; BitInput i = input) {
            ...
            final float[] expected = new float[randomValueForUnsignedInt(lengthSize)];
            for (int j = 0; j < expected.length; j++) {
                expected[j] = current().nextFloat();
            }
            new Float32ArrayWriter(lengthSize).write(o, expected);
            o.align();
            o.flush();
            final float[] actual = new Float32ArrayReader(lengthSize).read(i);
            i.align();
